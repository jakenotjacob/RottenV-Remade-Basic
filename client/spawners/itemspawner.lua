...
Citizen.CreateThread(function()
	while loaded == true do
		...
		if localWeapons < 5 or localFood < 2 then
			...
			for i, pickupInfo in ipairs(spawnedPickups) do
				if pickupInfo.spawned == false then
					if DistanceBetweenCoords(posX, posY, posZ, pickupInfo.x, pickupInfo.y, pickupInfo.z) < 300.0 then
						local pickup = 0
						if pickupInfo.pickupItem ~= nil then
							pickup = CreatePickupRotate(GetHashKey("PICKUP_PORTABLE_PACKAGE"), pickupInfo.x, pickupInfo.y, pickupInfo.z, 0.0, 0.0, 0.0, 512, 1, 24, 24, true, GetHashKey("PICKUP_PORTABLE_PACKAGE"))
						else
							pickup = CreatePickupRotate(pickupInfo.weapon, pickupInfo.x, pickupInfo.y, pickupInfo.z, 0.0, 0.0, 0.0, 512, pickupInfo.chance, 24, 24, true, pickupInfo.weapon)
						end
